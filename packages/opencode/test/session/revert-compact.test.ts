...
describe("revert + compact workflow", () => {
  bulletproofTest("should properly handle compact command after revert", async () => {
  ...
  bulletproofTest("should properly clean up revert state before creating compaction message", async () => {
    await using tmp = await tmpdir({ git: true })
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
