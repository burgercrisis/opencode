...
describe("Comprehensive Functional Tool Tests - Fixed", () => {
  let testDir: string
  ...
  test("WriteTool - file creation and overwriting", async () => {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        // Need to read first for file time tracking before editing
        await read.execute({ filePath: testFile }, ctx)
  ...
  test("BatchTool - concurrent execution", async () => {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        // Test with a simpler tool that's definitely available
        ...
        expect(result.results[0].output).toContain("batch1.txt")
        expect(result.results[1].output).toContain("batch2.txt") 
        expect(result.results[2].output).toContain("batch3.txt")
      }
    })
  })

  test("MultiEditTool - multiple edits on single file", async () => {
    await Instance.provide({
      directory: testDir,
      fn: async () => {
        ...
        expect(content).toContain("Welcome world") // unchanged
      }
    })
  })
  ...
  test("Error handling and edge cases", async () => {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        // When oldString not found, it should have 0 appliedEdits and 1 failedEdits
        expect(result.metadata.appliedEdits).toBe(0)
        expect(result.metadata.failedEdits).toBe(1)
        expect(result.output).toContain("Could not find oldString")
      }
    })
  })
})
