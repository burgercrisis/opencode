...
describe("InvalidTool", () => {
  ...
  bulletproofTest("should require tool parameter", async () => {
    const init = await InvalidTool.init()
    const parsed = init.parameters.safeParse({ error: "some error" })
    expect(parsed.success).toBe(false)
  })

  bulletproofTest("should require error parameter", async () => {
    const init = await InvalidTool.init()
  ...
  bulletproofTest("should execute and return error message", async () => {
    const init = await InvalidTool.init()
