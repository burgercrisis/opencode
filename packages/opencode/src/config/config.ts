...
export namespace Config {
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      throw new InvalidError({ path: item, issues: parsed.error.issues, message: parsed.error.message })
    }
    return result
  }
  ...
  export const Info = z
    .object({
      ...
      subagent_model_strategy: z
        .object({
          enabled: z
            .boolean()
            .optional()
            .describe("Enable intelligent subagent model selection"),
          global_allowlist: z
            .array(z.string())
            .optional()
            .describe("Whitelist of models subagents can use"),
          default_behavior: z
            .enum(["inherit", "fallback"])
            .default("inherit")
            .describe("Default behavior when disabled"),
          quality_preference: z
            .number()
            .min(0)
            .max(100)
            .default(50)
            .describe("Cost vs quality balance (0=cost-focused, 100=quality-focused)"),
          task_specific_overrides: z
            .record(
              z.string(),
              z.object({
                quality_preference: z.number().min(0).max(100),
                model_overrides: z.array(z.string()).optional(),
                cost_threshold: z.number().optional(),
              })
            )
            .optional()
            .describe("Per-task type configuration"),
          performance_learning: z
            .object({
            ...
            .optional()
            .describe("Performance learning settings"),
        })
        .optional()
        .describe("Intelligent subagent model selection strategy"),
      provider: z
  ...
  async function loadFile(filepath: string): Promise<Info> {
    ...
    let text = await Bun.file(filepath)
      ...
      .catch((err) => {
        ...
        throw new JsonError({ path: filepath, message: err.message })
      })
    if (!text) return {}
        ...
        const fileContent = (
          await Bun.file(resolvedPath)
            ...
            .catch((error) => {
              ...
              throw new InvalidError({ path: configFilepath, message: errMsg })
            })
        ).trim()
  ...
  export async function directories() {
    return (await state()).directories
  }
}
