...
export namespace LLMConcurrencyMachine {
  ...
  function getRoot() {
    return path.join(Global.Path.state, "llm-concurrency")
  }

  function getLeases() {
    return path.join(getRoot(), "leases")
  }
  ...
  export async function init() {
    ...
    log.info("cleaning up orphaned leases", { count: entries.length })
    ...
    for (const name of entries) {
      ...
      if (!exists) {
        ...
        log.info("removing orphaned lease", { pid, file: name })
  ...
  export async function snapshot(limits: Limits): Promise<Snapshot> {
    ...
    const entries = await fs.readdir(getLeases()).catch(() => [] as string[])
    ...
    for (const name of entries) {
      ...
      const filepath = path.join(getLeases(), name)
  ...
  async function createLease(input: { providerID: string; modelName: string; sessionID: string }) {
    await fs.mkdir(getLeases(), { recursive: true })

    const id = randomUUID()
    const filepath = path.join(getLeases(), `lease_${process.pid}_${id}.json`)
    const body = {
      providerID: input.providerID,
      modelName: input.modelName,
      sessionID: input.sessionID,
