  ...
  export function fromError(e: any, ctx: { providerID: string }) {
    switch (true) {
      ...
      case MessageV2.OutputLengthError.isInstance(e):
        return (e as any).toObject()
      case LoadAPIKeyError.isInstance(e):
        const errLoad = e as LoadAPIKeyError
        return new MessageV2.AuthError(
          {
            providerID: ctx.providerID,
            message: errLoad.message,
          },
          "",
        ).toObject()
...
<<<<<<< HEAD
      ...
      case APICallError.isInstance(e):
        ...
        const message = iife(() => {
          let msg = err.message
          if (msg === "") {
            if (err.responseBody) return err.responseBody
            if (err.statusCode) {
              const statusErr = STATUS_CODES[err.statusCode]
