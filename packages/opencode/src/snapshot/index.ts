...
export namespace Snapshot {
  ...
  export async function diffFull(from: string, to: string): Promise<FileDiff[]> {
    ...
    const show = async (hash: string, file: string) => {
      const response = await import { $ } from "bun"
import path from "path"
import fs from "fs/promises"
import { Log } from "../util/log"
import { Global } from "../global"
...
export namespace Snapshot {
  ...
  export type FileDiff = z.infer<typeof FileDiff>
git -c core.autocrlf=false -c core.quotepath=false --git-dir ${git} --work-tree ${Instance.worktree} show ${hash}:${file}`
        .quiet()
        .nothrow()
    ...
    const statusResult = await import { $ } from "bun"
import path from "path"
import fs from "fs/promises"
import { Log } from "../util/log"
import { Global } from "../global"
...
export namespace Snapshot {
  ...
  export type FileDiff = z.infer<typeof FileDiff>
git -c core.autocrlf=false -c core.quotepath=false --git-dir ${git} --work-tree ${Instance.worktree} diff --no-ext-diff --name-status --no-renames ${from} ${to} -- .`
      .quiet()
      .cwd(Instance.directory)
      .nothrow()
      .text()
...
export namespace Snapshot {
  ...
  export type FileDiff = z.infer<typeof FileDiff>
git -c core.autocrlf=false -c core.quotepath=false --git-dir ${git} --work-tree ${Instance.worktree} diff --no-ext-diff --no-renames --numstat ${from} ${to} -- .`
        .quiet()
        .cwd(Instance.directory)
        .nothrow()
        .text()
