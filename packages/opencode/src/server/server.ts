...
<<<<<<< HEAD
        if (err && typeof err === 'object' && 'name' in err && 'toObject' in err && typeof err.toObject === 'function') {
          ...
          if (namedError.name === 'StorageNotFoundError') status = 404
...
<<<<<<< HEAD
        ...
        describeRoute({
          ...
          description: "Revert changes to specific files in a session while keeping other changes.",
          operationId: "session.revertFiles",
          responses: {
            200: {
              description: "Updated session",
              content: {
        ...
        validator("json", SessionRevert.FileRevertInput.omit({ sessionID: true })),
        async (c) => {
          const sessionID = c.req.valid("param").sessionID
          log.info("revert files", c.req.valid("json"))
          const session = await SessionRevert.revertFiles({
            sessionID,
            ...c.req.valid("json"),
          })
          return c.json(session)
...
<<<<<<< HEAD
          try {
            ...
            const enabled = config.enabled_providers ? new Set(config.enabled_providers) : new Set()

            const allProviders = await ModelsDev.get()
            if (!allProviders) {
              console.error("ModelsDev.get() returned undefined")
              return
            }
