  ...
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      ...
      if (isUrl) {
        ...
        const enterpriseMatch = args.file.match(/https?:\/\/share\.opencode\.ai\/share\/([a-zA-Z0-9_-]+)/)

        const isLegacy = !!legacyMatch
        const isEnterprise = !!enterpriseMatch
        ...
        if (!slug) {
          ...
          process.stdout.write(EOL)
          process.stdout.write(`  - https://share.opencode.ai/share/<slug>`)
          process.stdout.write(EOL)
          return
        }

        if (isEnterprise) {
          const response = await fetch(`https://share.opencode.ai/api/share/${slug}/data`)
          if (!response.ok) {
