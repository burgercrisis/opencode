...
| `GET`    | `/session/:id/diff`                      | Get the diff for this session         | returns <a href={typesUrl}><code>FileDiff[]</code></a>                              |
| `POST`   | `/session/:id/summarize`                 | Summarize the session                 | body: `{ providerID, modelID }`, returns `boolean`                                  |
| `POST`   | `/session/:id/revert`                    | Revert a message                      | body: `{ messageID, partID? }`, returns <a href={typesUrl}><code>Session</code></a> |
| `POST`   | `/session/:id/unrevert`                  | Restore all reverted messages         | Returns <a href={typesUrl}><code>Session</code></a>                                 |
| `POST`   | `/session/:id/permissions/:permissionID` | Respond to a permission request       | body: `{ response }`, returns `boolean`                                             |
